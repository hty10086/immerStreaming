...
class InterfaceController: WKInterfaceController, HKWorkoutSessionDelegate, WorkoutManagerDelegate {
    ...
    override func willActivate() {
        ...
        guard let quantityType2 = HKQuantityType.quantityType(forIdentifier: .heartRateVariabilitySDNN) else { return }
        print("quantityType init")
        let dataTypes = Set(arrayLiteral: quantityType)
        let readingTypes:Set<HKObjectType> = [quantityType, quantityType2]
    ...
    func workoutDidStart() {
        ...
        // new query
        hrvQuery = HKObserverQuery(sampleType: hrvSampleType!, predicate: nil) { (_, _, error) in
            if let error = error {
                print("currentQuery error: \(error.localizedDescription)")
                return
            }
            
            self.fetchLatestHeartRateVariabilitySample() { (sample) in
                DispatchQueue.main.async {
                    let hrv = sample.quantity.doubleValue(for: self.hrvUnit)
                    print("Heart Rate Variability Sample: \(hrv)")
                    self.wcSession.transferUserInfo(["hrvSDNN": Int(hrv)])
        ...
        hrvQuery != nil ? healthStore.execute(hrvQuery!) : print("error: hrvQuery nil")
    ...
    func fetchLatestHeartRateVariabilitySample(completionHandler: @escaping (_ sample: HKQuantitySample) -> Void) {
        guard let sampleType = HKObjectType.quantityType(forIdentifier: HKQuantityTypeIdentifier.heartRateVariabilitySDNN) else {
            return
        }
    ...
    func updateHeartRate(value: Double) {
        ...
        let hrRange = heartRate - 0...heartRate + 0
