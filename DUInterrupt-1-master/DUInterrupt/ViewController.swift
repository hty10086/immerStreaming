...
class ViewController: UIViewController {
    ...
    let manager = SocketManager(socketURL: URL(string: "https://www.immerstreaming.com")!, config: [.log(true), .compress])
    ...
    override func viewDidLoad() {
        ...
        let timer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { timer in
            ...
            let ts = "\(hour):\(minutes):\(seconds)"
            self.socket.emit("send-message", ["hr": self.hrlabel.text, "user": self.partiID, "TimeStamp": ts, "HRV":self.hrv])
            
        }
        
//        let timer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { timer in
//            self.socket.emit("send-message", with: [self.hrlabel.text])
//        }
    }
...
extension ViewController {
    ...
    func processDayHR(_ results: [HKQuantitySample]) {
        ...
        DispatchQueue.main.async {
            ...
            self.hrlabel.text = (String)(newHeartRate)
            //self.socket.emit("send-message", with: [(String)(newHeartRate)])
...
extension ViewController: WCSessionDelegate {
    ...
    func session(_ session: WCSession, didReceiveUserInfo userInfo: [String : Any] = [:]) {
        ...
        if(userInfo["hrvSDNN"] as? Int == nil){
            print("No heart rate variability detected")
        }
        else{
            //print("NEW HEART RATE: ", userInfo["message"] as? String ?? "NULL")
            print("NEW HEART RATE Variability: ", userInfo["hrvSDNN"] as! Int)
            DispatchQueue.main.async {
                //self.hrlabel.text = (String)(userInfo["hrvSDNN"] as! Int)
                print("HRV: ", (String)(userInfo["hrvSDNN"] as! Int))
                self.hrv = userInfo["hrvSDNN"] as! Int
                print("Setting label to new heart rate")
            }
        }
    }
